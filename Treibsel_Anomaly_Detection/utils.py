...
def binary_dice_coefficient(prediction: Tensor, target: Tensor, smooth: int = 1) -> float:
    """Method for calculating the dice-coefficient between prediction and target.

    Args:
        prediction (Tensor): Prediction generated by model.
        target (Tensor): Target to compare to.
        smooth (int, optional): Smoothing factor. Defaults to 1.

    Returns:
        float: Calculated dice-coefficient.
    """
...
def generate_metrics(loader, model, device: str = "cpu") -> dict[str, float]:
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            dice += binary_dice_coefficient(preds, y)
